...
class VirtualStickFragment : DJIFragment() {
    ...
    private fun initBtnClickListener() {
        ...
        binding?.autonomous?.setOnClickListener {
            startDataReception { responseData ->
                responseData?.let {
                    virtualStickVM.virtualStickAdvancedParam.value?.let { param ->
                        Log.d("VirtualStickParam", "param: $param")  // Debug log
                        println("param: $param")  // Print to console (not recommended for production)
    ...
    private fun startDataReception(callback: (String?) -> Unit) {
        val client = OkHttpClient()
        val request = Request.Builder()
            .url("http://10.170.0.234:5000/data")  // Replace with actual server IP
            .build()
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    binding?.virtualStickInfoTv?.text = "Received: $responseData"
                    storeDataLocally(responseData ?: "")
                }
            } catch (e: IOException) {
                e.printStackTrace()
