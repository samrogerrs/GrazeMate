...
const ssSwiper = function() {
   ...
   const ssMoveTo = function() {
    ...
    moveTo.move = function (target) {
        const offset = 200; // Adjust this value to control how far above the target you want to scroll
        const isFeaturesSection = target.closest('#features') !== null; // Check if target is in the features section
        
        const targetPosition = target.offsetTop - (isFeaturesSection ? offset : 0); // Apply offset only for the features section
        
        // Animate scroll with offset only if the target is within the features section
        window.scrollTo({
            top: targetPosition,
            behavior: 'smooth'
        });
